...
class Checkpoint(Action):
    def run(self, username, password, ip, host_name, blacklist_group):
        ...
        new_host = requests.post('https://192.168.1.24:443/web_api/add-host', json=host_payload, headers=call_headers, verify=False)
        if 'meta-info' in new_host.json():
            validation = new_host.json()['meta-info']['validation-state']
            published = False
            publish = requests.post('https://192.168.1.24:443/web_api/publish', json={}, headers=call_headers, verify=False)
            while not published:
